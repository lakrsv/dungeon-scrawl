namespace Tiles.Rule_Tile.Scripts
{
    using System;
    ...
    public class RuleTile : TileBase
    {
                    ...
                    switch (rule.m_Output)
                    {
                        ...
                        case TilingRule.OutputSprite.Random:
                            var index = Mathf.Clamp(
                                Mathf.FloorToInt(
                                    GetPerlinValue(position, rule.m_PerlinScale, 100000f) * rule.m_Sprites.Length),
                                0,
                                rule.m_Sprites.Length - 1);
                            tileData.sprite = rule.m_Sprites[index];
                            if (rule.m_RandomTransform != TilingRule.Transform.Fixed)
                                transform = ApplyRandomTransform(
                                    rule.m_RandomTransform,
                                    transform,
                                    rule.m_PerlinScale,
                                    position);
                            break;
        ...
        public override void RefreshTile(Vector3Int location, ITilemap tileMap)
        {
            if (m_TilingRules != null && m_TilingRules.Count > 0)
                for (var y = -1; y <= 1; y++)
                for (var x = -1; x <= 1; x++) base.RefreshTile(location + new Vector3Int(x, y, 0), tileMap);
            else base.RefreshTile(location, tileMap);
        }
            ...
            for (var x = -1; x <= 1; x++)
                if (x != 0 || y != 0)
                {
                    ...
                    if (rule.m_Neighbors[index] == TilingRule.Neighbor.This && tile != this
                        || rule.m_Neighbors[index] == TilingRule.Neighbor.NotThis && tile == this) return false;
                }
