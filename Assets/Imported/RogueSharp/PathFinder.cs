// // --------------------------------------------------------------------------------------------------------------------
// // <copyright file="PathFinder.cs" author="Lars-Kristian Svenoy" company="Cardlike">
...
namespace Imported.RogueSharp
{
    ...
    using Imported.RogueSharp.Algorithms;
    ...
    public class PathFinder
    {
        ...
        public PathFinder(IMap map)
        {
            ...
            foreach (var cell in _map.GetAllCells())
                ...
                {
                    var v = IndexFor(cell);
                    foreach (var neighbor in _map.GetBorderCellsInDiamond(cell.X, cell.Y, 1))
                        if (neighbor.IsWalkable)
                        {
                            var w = IndexFor(neighbor);
        ...
        /// <summary>
        ///     Constructs a new PathFinder instance for the specified Map that will consider diagonal movement by using the
        ///     specified diagonalCost
        /// </summary>
        /// <param name="map">The Map that this PathFinder instance will run shortest path algorithms on</param>
        ...
        public PathFinder(IMap map, double diagonalCost)
        {
            ...
            foreach (var cell in _map.GetAllCells())
                ...
                {
                    var v = IndexFor(cell);
                    foreach (var neighbor in _map.GetBorderCellsInSquare(cell.X, cell.Y, 1))
                        if (neighbor.IsWalkable)
                        {
                            var w = IndexFor(neighbor);
        ...
        {
            var shortestPath = TryFindShortestPath(source, destination);

            if (shortestPath == null)
                throw new PathNotFoundException(
                    string.Format(
                        "Path from ({0}, {1}) to ({2}, {3}) not found",
                        source.X,
                        source.Y,
                        destination.X,
                        destination.Y));
        ...
        /// <exception cref="ArgumentNullException">Thrown when source or destination is null</exception>
        /// <returns>
        ///     Returns a shortest Path containing a list of Cells from a specified source Cell to a destination Cell. If no
        ///     path is found null will be returned
        /// </returns>
        ...
        private IEnumerable<ICell> ShortestPathCells(ICell source, ICell destination)
        {
            ...
            {
                yield return source;
                foreach (var edge in path) yield return CellFor(edge.To);
            }
        }
    }
}