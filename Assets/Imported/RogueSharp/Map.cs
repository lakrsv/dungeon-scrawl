// // --------------------------------------------------------------------------------------------------------------------
// // <copyright file="Map.cs" author="Lars-Kristian Svenoy" company="Cardlike">
...
namespace Imported.RogueSharp
{
    ...
    /// <summary>
    ///     A Map represents a rectangular grid of Cells, each of which has a number of properties for determining walkability,
    ///     field-of-view and so on
    ///     The upper left corner of the Map is Cell (0,0) and the X value increases to the right, as the Y value increases
    ///     downward
    /// </summary>
    public class Map : IMap
    {
        ...
        /// </remarks>
        /// <param name="mapCreationStrategy">
        ///     A class that implements IMapCreationStrategy and has CreateMap method which defines
        ///     algorithms for creating interesting Maps
        /// </param>
        ...
        /// <summary>
        ///     Performs a field-of-view calculation with the specified parameters and appends it any existing field-of-view
        ///     calculations.
        ///     Field-of-view (FOV) is basically a calculation of what is observable in the Map from a given Cell with a given
        ///     light radius.
        /// </summary>
        /// <example>
        ///     When a character is holding a light source in a large area that also has several other sources of light such as
        ///     torches along the walls
        ///     ComputeFov could first be called for the character and then AppendFov could be called for each torch to give us the
        ///     final combined FOV given all the light sources
        /// </example>
        ...
        /// <param name="radius">
        ///     The number of Cells in which the field-of-view extends from the origin Cell. Think of this as the
        ///     intensity of the light source.
        /// </param>
        /// <param name="lightWalls">
        ///     True if walls should be included in the field-of-view when they are within the radius of the
        ///     light source. False excludes walls even when they are within range.
        /// </param>
        ...
        /// <summary>
        ///     Get the Cell at the specified single dimensional array index using the formulas: x = index % Width; y = index /
        ///     Width;
        /// </summary>
        ...
        ///     Optional parameter defaults to true if not provided. True if line-of-sight is not blocked
        ///     by this Cell. False otherwise
        /// </param>
        /// <param name="isWalkable">
        ///     Optional parameter defaults to true if not provided. True if a character could walk across the
        ///     Cell normally. False otherwise
        /// </param>
        public void Clear(bool isTransparent, bool isWalkable)
        {
            foreach (var cell in GetAllCells()) SetCellProperties(cell.X, cell.Y, isTransparent, isWalkable);
        }
        ...
        ///     An IMap which must be of smaller size and able to fit in this destination Map at the specified
        ///     location
        /// </param>
        ...
        /// </param>
        /// <param name="left">
        ///     Optional parameter defaults to 0 if not provided. X location of the Cell to start copying parameters
        ///     to, starting with 0 as the farthest left
        /// </param>
        /// <param name="top">
        ///     Optional parameter defaults to 0 if not provided. Y location of the Cell to start copying parameters
        ///     to, starting with 0 as the top
        /// </param>
        ...
        {
            if (sourceMap == null) throw new ArgumentNullException("sourceMap", "Source map cannot be null");
            ...
            foreach (var cell in sourceMap.GetAllCells())
        ...
        {
            for (var y = 0; y < Height; y++) for (var x = 0; x < Width; x++) yield return GetCell(x, y);
        }

        /// <summary>
        ///     Get an IEnumerable of outermost border Cells in a circle around the center Cell up to the specified radius using
        ...
        public IEnumerable<ICell> GetBorderCellsInCircle(int xCenter, int yCenter, int radius)
        {
            ...
            {
                ICell cell;
                if (AddToHashSet(discovered, ClampX(xCenter + x), ClampY(yCenter + y), out cell)) yield return cell;
                if (AddToHashSet(discovered, ClampX(xCenter + x), ClampY(yCenter - y), out cell)) yield return cell;
                if (AddToHashSet(discovered, ClampX(xCenter - x), ClampY(yCenter + y), out cell)) yield return cell;
                if (AddToHashSet(discovered, ClampX(xCenter - x), ClampY(yCenter - y), out cell)) yield return cell;
                if (AddToHashSet(discovered, ClampX(xCenter + y), ClampY(yCenter + x), out cell)) yield return cell;
                if (AddToHashSet(discovered, ClampX(xCenter + y), ClampY(yCenter - x), out cell)) yield return cell;
                if (AddToHashSet(discovered, ClampX(xCenter - y), ClampY(yCenter + x), out cell)) yield return cell;
                if (AddToHashSet(discovered, ClampX(xCenter - y), ClampY(yCenter - x), out cell)) yield return cell;
        ...
        /// <summary>
        ///     Get an IEnumerable of outermost border Cells in a diamond (Rhombus) shape around the center Cell up to the
        ///     specified distance
        /// </summary>
        /// <param name="xCenter">X location of the center Cell with 0 as the farthest left</param>
        /// <param name="yCenter">Y location of the center Cell with 0 as the top</param>
        ...
        public IEnumerable<ICell> GetBorderCellsInDiamond(int xCenter, int yCenter, int distance)
        {
            ...
            var xMin = Math.Max(0, xCenter - distance);
            var xMax = Math.Min(Width - 1, xCenter + distance);
            var yMin = Math.Max(0, yCenter - distance);
            var yMax = Math.Min(Height - 1, yCenter + distance);

            ICell cell;
            if (AddToHashSet(discovered, xCenter, yMin, out cell)) yield return cell;
            if (AddToHashSet(discovered, xCenter, yMax, out cell)) yield return cell;
            for (var i = 1; i <= distance; i++)
            {
                if (AddToHashSet(
                    discovered,
                    Math.Max(xMin, xCenter - i),
                    Math.Min(yMax, yCenter + distance - i),
                    out cell)) yield return cell;
                if (AddToHashSet(
                    discovered,
                    Math.Max(xMin, xCenter - i),
                    Math.Max(yMin, yCenter - distance + i),
                    out cell)) yield return cell;
                if (AddToHashSet(
                    discovered,
                    Math.Min(xMax, xCenter + i),
                    Math.Min(yMax, yCenter + distance - i),
                    out cell)) yield return cell;
                if (AddToHashSet(
                    discovered,
        ...
        ///     Uses Bresenham's line algorithm to determine which Cells are in the closest approximation to a straight line
        ...
        /// <returns>
        ///     IEnumerable of Cells in a line from the Origin Cell to the Destination Cell which includes the Origin and
        ///     Destination Cells
        /// </returns>
        public IEnumerable<ICell> GetCellsAlongLine(int xOrigin, int yOrigin, int xDestination, int yDestination)
        {
            ...
            while (true)
            {
                ...
                if (xOrigin == xDestination && yOrigin == yDestination) break;
                var e2 = 2 * err;
                if (e2 > -dy)
                {
                    err = err - dy;
        ...
        /// <summary>
        ///     Get an IEnumerable of Cells in a circle around the center Cell up to the specified radius using Bresenham's
        ...
        public IEnumerable<ICell> GetCellsInCircle(int xCenter, int yCenter, int radius)
        {
            ...
            do
            {
                foreach (var cell in GetCellsAlongLine(xCenter + x, yCenter + y, xCenter - x, yCenter + y))
                    if (AddToHashSet(discovered, cell)) yield return cell;
                foreach (var cell in GetCellsAlongLine(xCenter - x, yCenter - y, xCenter + x, yCenter - y))
                    if (AddToHashSet(discovered, cell)) yield return cell;
                foreach (var cell in GetCellsAlongLine(xCenter + y, yCenter + x, xCenter - y, yCenter + x))
                    if (AddToHashSet(discovered, cell)) yield return cell;
                foreach (var cell in GetCellsAlongLine(xCenter + y, yCenter - x, xCenter - y, yCenter - x))
                    if (AddToHashSet(discovered, cell)) yield return cell;
        ...
        {
            foreach (var x in columnNumbers) for (var y = 0; y < Height; y++) yield return GetCell(x, y);
        }
        ...
        public IEnumerable<ICell> GetCellsInDiamond(int xCenter, int yCenter, int distance)
        {
            ...
            for (var j = distance; j >= 0 + i; j--)
            {
                ICell cell;
                if (AddToHashSet(
                    discovered,
                    Math.Max(xMin, xCenter - i),
                    Math.Min(yMax, yCenter + distance - j),
                    out cell)) yield return cell;
                if (AddToHashSet(
                    discovered,
                    Math.Max(xMin, xCenter - i),
                    Math.Max(yMin, yCenter - distance + j),
                    out cell)) yield return cell;
                if (AddToHashSet(
                    discovered,
                    Math.Min(xMax, xCenter + i),
        ...
        {
            foreach (var y in rowNumbers) for (var x = 0; x < Width; x++) yield return GetCell(x, y);
        }
        ...
        public IEnumerable<ICell> GetCellsInSquare(int xCenter, int yCenter, int distance)
        {
            ...
            for (var y = yMin; y <= yMax; y++) for (var x = xMin; x <= xMax; x++) yield return GetCell(x, y);
        }

        /// <summary>
        ///     Get the single dimensional array index for a Cell at the specified location using the formula: index = ( y * Width
        ///     ) + x
        /// </summary>
        ...
        {
            if (cell == null) throw new ArgumentNullException("cell", "Cell cannot be null");
        ...
        /// </summary>
        /// <remarks>
        ///     The explored property of a Cell can be used to track if the Cell has ever been in the field-of-view of a character
        ///     controlled by the player
        ///     This property will not automatically be updated based on FOV calculations or any other built-in functions of the
        ///     RogueSharp library.
        /// </remarks>
        /// <example>
        ///     As the player moves characters around a Map, Cells will enter and exit the currently computed field-of-view
        ///     This property can be used to keep track of those Cells that have been "seen" and could be used to show fog-of-war
        ///     type effects when rendering the map
        /// </example>
        ...
        /// </remarks>
        /// <example>
        ///     Field-of-view can be used to simulate a character holding a light source and exploring a Map representing a dark
        ///     cavern
        ...
        {
            if (state == null) throw new ArgumentNullException("state", "Map state cannot be null");
            ...
            foreach (var cell in GetAllCells())
            {
                ...
                if (cellProperties.HasFlag2(MapState.CellProperties.Visible)) inFov.Add(IndexFor(cell.X, cell.Y));
        ...
        public MapState Save()
        {
            ...
            foreach (var cell in GetAllCells())
            {
                ...
                if (cell.IsInFov) cellProperties |= MapState.CellProperties.Visible;
                if (cell.IsTransparent) cellProperties |= MapState.CellProperties.Transparent;
                if (cell.IsWalkable) cellProperties |= MapState.CellProperties.Walkable;
        ...
        public void SetCellExplored(int x, int y, bool isExplored)
        {
            _isExplored[x, y] = isExplored;
        }
        ...
        /// </summary>
        /// <param name="useFov">
        ///     True if field-of-view calculations will be used when creating the string represenation of the Map.
        ///     False otherwise
        /// </param>
        ...
        public string ToString(bool useFov)
        {
            ...
            var lastY = 0;
            foreach (var iCell in GetAllCells())
            {
                var cell = (Cell)iCell;
                if (cell.Y != lastY)
                {
